...
This repository is **not** the CalenFit codebase. It's a public write-up of the system design and the production engineering behind it — the thinking, not the proprietary source. CalenFit is a live AI fitness app; this document explains how its core works and the trade-offs I made to ship it reliably to real users.

---

## The problem

I wanted users to log workouts the way they naturally write them, not the way software wants to receive them.
...
To software, they're three incompatible inputs. Most fitness apps avoid this entirely by forcing rigid forms — dropdowns, steppers, fixed fields. That's reliable but it fights how people actually think about their training. The open question I wanted to test was simple: **can an LLM pipeline parse free-text workouts reliably enough to actually ship?**
...
The key design decision was **not** to ask one model to do everything. Two narrow prompts proved far more reliable than one large one — each model has a single, well-bounded responsibility, which makes its output easier to constrain and validate.

**Interpreter** — resolves *what exercise this is*. It handles abbreviations (`DL` → deadlift), shorthand, and misspellings, mapping messy free-text to a canonical exercise.

**Parser** — extracts *the structure*: sets, reps, loads, and other set-level data, normalizing the many ways people write the same thing (`3x8`, `8/8/8`, `8@100`) into one schema.
...
This is the part I'm proudest of: the unavoidable latency of LLM inference becomes a **human-in-the-loop validation step**. It guarantees data integrity at the source without the user ever feeling a delay. Validation isn't bolted on after the fact — it's designed into the interaction.
...
LLM tail latency is the enemy of a "feels instant" logging experience. I measured the real distribution across many runs and found an upper-percentile threshold around **~9s**. Calls that exceed that threshold are caught and **re-issued**, which clips the worst-case wait rather than letting an unlucky request hang the interaction. Combined with the concurrent interpreter/parser design and the confirmation-card-during-inference pattern, the perceived latency stays low.

### Cost

I **benchmarked candidate models** against each other on three axes — cost, accuracy, and latency — and picked the best trade-off rather than defaulting to the most capable model. That benchmarking cut inference cost roughly **2×** versus the model I started with, with no meaningful accuracy loss for this task.
...
The model was not the hard part. The real engineering was everything around it:

- **Coordinating the agents** so two specialized models cooperate reliably.
- **Designing validation as UX** so data integrity and perceived speed reinforce each other instead of trading off.
- **The reliability and cost discipline** — gateway, retries, fallback, latency clipping, model benchmarking, observability — that are the actual requirements for shipping an LLM product to real users.

That's the difference between a demo and a production app, and it's where almost all the work lived.
